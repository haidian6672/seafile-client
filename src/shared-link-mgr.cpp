...
void SharedLinkManager::getSharedLink(const SharedLinkRequestParams& params)
{
    if (params.internal) {
        ...
        connect(req, SIGNAL(failed()),
                this, SLOT(onGetSharedLinkFailed()));
...
void SharedLinkManager::onGetSharedLinkFailed()
{
...
void SharedLinkManager::onDeleteSharedLinkFailed()
{
...
void SharedLinkManager::createSharedLink(const SharedLinkRequestParams& params)
{
    if (!params.advanced) {
        ...
        connect(req, SIGNAL(failed()),
                this, SLOT(onCreateSharedLinkFailed()));
        req->send();
        reqs_.push_back(req);
    } else {
...
void SharedLinkManager::onCreateSharedLinkFailed()
{
